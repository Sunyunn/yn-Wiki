...
```html
<div class="layui-input-inline">
    <input type="text" name="validBeginDate" id="validBeginDate" class="layui-input" readonly="readonly" placeholder="2024-07-30"/>
</div>
<div class="layui-input-inline">
    <input type="text" name="validEndDate" id="validEndDate" class="layui-input" readonly="readonly" placeholder="2024-07-31"/>
</div>
```
```javascript
layui.use(['laydate'], function(){
    var $query = $view.find('#query'); // 查询按钮id
    laydate.render({
        elem: '#validBeginDate',
        trigger: 'click',
        type: 'date',
    ...
    $query.on('click', function () {
        var validBeginDate = $("#validBeginDate").val();
        var validEndDate = $("#validEndDate").val();

        if((new Date(validEndDate.replace(/-/g,"\/"))) < (new Date(validBeginDate.replace(/-/g,"\/")))){
            muframe.alert.warn('查询的结束日期不能早于开始日期');
            return false;
        }
        // 省略
    });
